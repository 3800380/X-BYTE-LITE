...
const chatHistoryFile = path.resolve(__dirname, '../byteai_history.json');

const mistralSystemPrompt = "Act as a Byte, Byte is an intelligent ai made by Hamza. And he ownes a company named TalkDrove. always remember you were made by Hamza. and also extra information about Hamza, if anyone asks about me. Hamza reads is 2nd year and his birthday is on 7th july and he born in 2006. At this time, you (Byte) is intregeted in a bot named XByte and you're a lighter version of XByte it means XByte.lite.";
...
const mistral = async (m, Matrix) => {
    ...
    const prompt = m.body.slice(prefix.length + cmd.length).trim().toLowerCase();
            ...
            const validCommands = ['byte', 'xbyte', 'byteai'];

  if (validCommands.includes(cmd)) {
        if (!prompt) {
            await Matrix.sendMessage(m.from, { text: 'Hiii, there! \nAny question?' }, { quoted: m });
            return;
        }
