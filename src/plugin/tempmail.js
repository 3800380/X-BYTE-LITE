...
const tempMailCommand = async (m, Matrix) => {
    ...
    if (cmd === 'tempmail') {
        try {
            ...
            const genResponse = await fetch('https://www.1secmail.com/api/v1/?action=genRandomMailbox&count=1');
        ...
        try {
            ...
            const inboxResponse = await fetch(`https://www.1secmail.com/api/v1/?action=getMessages&login=${email}`);
